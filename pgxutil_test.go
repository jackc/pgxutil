...
func TestQueueUpdateRow(t *testing.T) {
	t.Parallel()
	...
	age int
		...
		batch := &pgx.Batch{}
		pgxutil.QueueUpdateRow(batch, pgx.Identifier{"t"}, map[string]any{"age": 42}, nil)
		err = conn.SendBatch(ctx, batch).Close()
		require.ErrorIs(t, err, pgx.ErrNoRows)
		...
		batch = &pgx.Batch{}
		pgxutil.QueueUpdateRow(batch, pgx.Identifier{"t"}, map[string]any{"age": 70}, map[string]any{"name": "John"})
		err = conn.SendBatch(ctx, batch).Close()
		require.NoError(t, err)

		batch = &pgx.Batch{}
		pgxutil.QueueUpdateRow(batch, pgx.Identifier{"t"}, map[string]any{"age": 70}, nil)
		err = conn.SendBatch(ctx, batch).Close()
		require.ErrorContains(t, err, "too many rows")
	})
}
...
func TestQueueUpdateRowReturning(t *testing.T) {
	t.Parallel()
	...
	age int
		...
		batch := &pgx.Batch{}
		var person *Person
		pgxutil.QueueUpdateRowReturning(batch, pgx.Identifier{"t"}, map[string]any{"age": 42}, nil, "*", pgx.RowToAddrOfStructByPos[Person], &person)
		err = conn.SendBatch(ctx, batch).Close()
		require.ErrorIs(t, err, pgx.ErrNoRows)
		require.Nil(t, person)
		...
		batch = &pgx.Batch{}
		pgxutil.QueueUpdateRowReturning(batch, pgx.Identifier{"t"}, map[string]any{"age": 70}, nil, "*", pgx.RowToAddrOfStructByPos[Person], &person)
		err = conn.SendBatch(ctx, batch).Close()
		require.ErrorContains(t, err, "too many rows")
	})
}
